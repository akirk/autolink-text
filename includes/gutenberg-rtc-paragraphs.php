...
function gutenberg_rtc_build_last_word_replacement( array $state, Gutenberg_RTC_Completed_Paragraph $paragraph, string $replacement, int $client_id, int $start_clock ): ?array {
	...
	return gutenberg_rtc_build_text_replacement(
		$state,
		$paragraph,
		$word['start'],
		$word['length'],
...
/**
 * Builds update bytes for replacing a text range in a completed paragraph.
 *
 * @param array<string, mixed> $state State used to locate the source text item.
 * @return array<string, mixed>|null
 */
function gutenberg_rtc_build_text_replacement( array $state, Gutenberg_RTC_Completed_Paragraph $paragraph, int $start, int $length, string $original, string $replacement, int $client_id, int $start_clock ): ?array {
...
/**
 * Builds update bytes for wrapping a text range in a completed paragraph.
 *
 * @param array<string, mixed> $state State used to locate the source text item.
 * @return array<string, mixed>|null
 */
function gutenberg_rtc_build_text_wrap( array $state, Gutenberg_RTC_Completed_Paragraph $paragraph, int $start, int $length, string $opening_text, string $closing_text, int $client_id, int $start_clock ): ?array {
	if ( $start < 0 || $length <= 0 || '' === $opening_text || '' === $closing_text ) {
		return null;
	}
	...
	$open_origin        = $start > 0 ? $nodes[ $start - 1 ]['id'] : null;
...
function gutenberg_rtc_apply_decoded_update_to_paragraph_state( array &$state, array $decoded ): array {
	...
	foreach ( $structs as $struct ) {
		...
		if ( 'string' === ( $content['type'] ?? '' ) ) {
			$touched_block = gutenberg_rtc_note_text_item( $state, $struct, $id );
			if ( $touched_block ) {
 ...
 */
function gutenberg_rtc_note_text_item( array &$state, array $struct, string $id ): ?string {
	$content = isset( $struct['content'] ) && is_array( $struct['content'] ) ? $struct['content'] : array();
	...
	foreach ( $children as &$child_keys ) {
		usort(
			...
			static function ( string $a, string $b ) use ( $nodes ): int {
				$a_id    = $nodes[ $a ]['id'];
				$b_id    = $nodes[ $b ]['id'];
				$a_right = gutenberg_rtc_yjs_id_key( $nodes[ $a ]['right_origin'] ?? null );
				$b_right = gutenberg_rtc_yjs_id_key( $nodes[ $b ]['right_origin'] ?? null );
